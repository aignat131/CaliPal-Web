...
export default function InviteFriendsSheet({ battleId, onClose }: Props) {
  const { user } = useAuth()
  const t = useT()
  ...
  async function handleInvite(friend: FriendEntry) {
    ...
    await createNotification(
      ...
      'BATTLE_INVITE',
      'Provocare la luptă!',
      `${user.displayName ?? 'Cineva'} te provoacă la o luptă!`,
      battleId,
      user.uid,
    )
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-white/90">{t('battle.invite_friends')}</h2>
